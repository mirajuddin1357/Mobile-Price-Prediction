...
try:
    ...
    st.sidebar.write("Cannot list files")
...
# Load dataset - WITH COMPLETE ERROR HANDLING
try:
    # Try all possible CSV files
    csv_files = ['smartphone_cleaned_v5.csv', 'smartphone_cleaned_v1.csv', 'smartphone.csv']
    dfen = None
    
    for csv_file in csv_files:
        try:
            dfen = pd.read_csv(csv_file)
            st.sidebar.success(f"Dataset loaded from {csv_file}!")
            break
        except:
            continue
    
    if dfen is None:
        st.sidebar.error("Could not load any dataset file!")
        # Create empty dataframe to prevent errors
        dfen = pd.DataFrame(columns=['brand_name', 'model', 'processor_brand', 'os'])
    else:
        # Process the dataframe only if it was loaded successfully
...
# ===============================================================
    ...
    if x is None:
        ...
        st.info("Please ensure 'smartphone_price_model.pkl' file exists in the project directory.")
    else:
        # Input Form
        try:
            # Try to load dataset for the prediction page
            csv_files = ['smartphone_cleaned_v5.csv', 'smartphone_cleaned_v1.csv', 'smartphone.csv']
            df = None
            ...
            if df is None:
                st.error("Could not load dataset for prediction.")
                st.stop()
            ...
            st.error(f"Error in prediction form: {e}")
            st.info("Please check if all required data files are available.")
...
# ===============================================================
    ...
    try:
        ...
        st.error("Documentation image not found.")
